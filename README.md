...
| 养护任务 | `/tasks` | 任务登记（编号按日生成）、按状态/类型/优先级/绿地/计划日期区间/逾期筛选、状态流转（待执行→进行中→已完成/已取消，校验前置条件，已取消为终态）、任务详情（执行进度 + 状态变更记录） |
...
| `maintenance_task_status_log` | 任务状态流转日志 | `task_id`、`from_status`、`to_status`、`source`(手动/记录联动)、`note`、`created_at` |
